...
namespace BWC.DBCore.Repositories.Base
{
         ...
         where T : class
    {
        ...
        public IEnumerable<T> GetWhere(string whereclause) => Context.Set<T>().Where(whereclause).ToList();

        public IEnumerable<T> GetWhere1(string whereclause, string includes)
        {
            return Context.Set<T>().Include(includes).Where(whereclause).ToList();
        }
